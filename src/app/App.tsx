...
export default function App() {
    ...
    (row: number, col: number) => {
      ...
      setGrid((prevGrid) => {
        ...
        if (currentTool === "start") {
          ...
          if (startPoint) {
            newGrid[startPoint.row][startPoint.col] = {
              type: "empty",
            };
          }
          ...
          if (endPoint) {
            newGrid[endPoint.row][endPoint.col] = {
              type: "empty",
            };
          }
  ...
  const runVisualization = useCallback(async () => {
    ...
    // Add a small delay to show the reset
    ...
    // Calculate delay based on speed (invert so higher speed = faster animation)
    const visitedDelay = Math.max(10, 150 - speed);
    const pathDelay = Math.max(20, 200 - speed * 1.5);
    ...
    for (let i = 0; i < result.visited.length; i++) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, visitedDelay),
      );
    }
    ...
    // Animate path and move robot along it
    if (result.path.length > 0) {
      ...
      for (let i = 0; i < result.path.length; i++) {
        ...
        let direction = 0;
        if (nextNode) {
          const dx = nextNode.col - node.col;
          const dy = nextNode.row - node.row;
        ...
        setRobots((prevRobots) => {
          ...
          if (newRobots[0]) {
            newRobots[0].position = node;
            newRobots[0].direction = direction;
            newRobots[0].task = "Moving to destination";
            newRobots[0].battery = Math.max(0, 85 - i);
          }
          return newRobots;
        });
      ...
      setRobots((prevRobots) => {
        ...
        if (newRobots[0]) {
          ...
          newRobots[0].battery = Math.max(
            0,
            85 - result.path.length,
          );
        }
        return newRobots;
      });
    } else {
      setAlgorithmLog((prev) => [...prev, "No path found!"]);
    }
  ...
  const calculateCellSize = () => {
    ...
    const maxWidth = Math.max(
      800,
      window.innerWidth * 0.7 - 48,
    );
    return Math.max(
      15,
      Math.min(30, Math.floor(maxWidth / cols)),
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white py-6 px-8 shadow-lg">
        <h1 className="mb-1">
          Warehouse Robot Pathfinding Visualizer
        </h1>
        <p className="text-gray-300">
          Amazon Fulfillment Center Operations Dashboard
        </p>
      </div>
